        ...
        for (int i = 0; i < expression.size(); i++) {
            ...
            if(isOperator(token)){
                while(isFunction(operators.top()) ||
                      (isOperator(operators.top() && (getPrecedence(operators.top()) > getPrecedence(token)))) ||
                      ((isOperator(operators.top() && ((getPrecedence(operators.top()) == getPrecedence(token)) && isLeftAssociative(operators.top())))) &&
                       (operators.top() != '('))) {
